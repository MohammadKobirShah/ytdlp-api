...
.h-btn{color:var(--t2);text-decoration:none;padding:8px;border-radius:var(--rx);transition:.2s;
...
.input-row input{flex:1;background:var(--input);border:1.5px solid var(--border);
  border-radius:var(--rs);padding:14px 16px;color:var(--t1);font-size:15px;outline:none;transition:border .2s}
...
.btn-fetch{width:100%;padding:14px;border:none;border-radius:var(--rs);
  ...
  display:flex;align-items:center;justify-content:center;gap:8px;transition:.2s;letter-spacing:.3px}
...
.dur-badge{position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,.85);
  color:#fff;padding:3px 8px;border-radius:4px;font-size:12px;font-weight:700;letter-spacing:.3px}
...
.mode-tab{flex:1;padding:10px;text-align:center;cursor:pointer;z-index:1;position:relative;
  ...
  color:var(--t2);font-size:14px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:6px}
...
.fmt-row{display:flex;align-items:center;gap:12px;background:var(--card);
  border:1.5px solid var(--border);border-radius:var(--rs);padding:12px 14px;cursor:pointer;transition:.2s}
...
.dl-icon{width:42px;height:42px;border-radius:6px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
.dl-btn{width:38px;height:38px;border:none;border-radius:6px;cursor:pointer;
...
.cookie-badge{font-size:9px;font-weight:700;padding:2px 5px;border-radius:3px;
  background:rgba(0,230,118,.15);color:var(--green);margin-left:2px;vertical-align:super}
.cookie-badge.off{background:rgba(255,82,82,.12);color:var(--red)}
...
<header>
  <div class="h-left">
    ...
    <span id="cookie-badge" class="cookie-badge off">NO COOKIES</span>
  </div>
  <div class="h-right">
    <button class="h-btn" id="btn-cookies" title="Upload cookies.txt">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm0 14v-4m0-4h.01"/></svg>
    </button>
...
<script>
var S={tab:'home',mode:'audio',url:'',info:null,loading:false,preset:'128k',fmtId:null,fmts:[],task:null,cookiesLoaded:false};
var polls={};
...
var API={
  info:function(url){return fetch('/api/video/info?url='+encodeURIComponent(url)+'&include_raw=false').then(function(r){if(!r.ok)return r.json().then(function(e){throw new Error(e.detail||'Fetch failed')});return r.json();});},
  audio:function(url,preset){return fetch('/api/audio',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:url,preset:preset})}).then(function(r){if(!r.ok)return r.json().then(function(e){throw new Error(e.detail||'Start failed')});return r.json();});},
  video:function(url,format_id){var b={url:url};if(format_id)b.format_id=format_id;return fetch('/api/video',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}).then(function(r){if(!r.ok)return r.json().then(function(e){throw new Error(e.detail||'Start failed')});return r.json();});},
...
function updateCookieBadge(){
  var el=document.getElementById('cookie-badge');
  if(S.cookiesLoaded){el.textContent='COOKIES OK';el.className='cookie-badge';}
  else{el.textContent='NO COOKIES';el.className='cookie-badge off';}
}

function checkCookies(){
  API.cookiesStatus().then(function(d){S.cookiesLoaded=d.loaded;updateCookieBadge();}).catch(function(){S.cookiesLoaded=false;updateCookieBadge();});
}

function renderPresets(){
  var presets=[{id:'48k',label:'Low',sr:'22.0kHz',ch:'Mono'},{id:'64k',label:'Medium',sr:'22.0kHz',ch:'Mono'},{id:'128k',label:'HQ',sr:'44.1kHz',ch:'Stereo'},{id:'320k',label:'Best',sr:'48kHz',ch:'Stereo'}];
  document.getElementById('preset-grid').innerHTML=presets.map(function(p){
    return '<div class="preset-card'+(S.preset===p.id?' selected':'')+'" data-preset="'+p.id+'"><div class="p-check"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></div><div class="p-bitrate">'+p.id+'</div><div class="p-label">'+p.label+'</div><div class="p-detail">'+p.sr+' &middot; '+p.ch+'</div></div>';
  }).join('');
  document.querySelectorAll('.preset-card').forEach(function(el){el.addEventListener('click',function(){S.preset=el.dataset.preset;renderPresets();});});
}
...
function processFormats(formats){
  ...
  var byH={};
  src.forEach(function(f){var h=parseInt(f.resolution)||0;if(!h)return;if(!byH[h]||(f.tbr||0)>(byH[h].tbr||0))byH[h]=f;});
...
function renderFormats(){
  ...
  list.innerHTML=fmts.map(function(f){
    var res=f.resolution||'?';var ext=f.ext||'';var codec=f.vcodec?f.vcodec.split('.')[0].substring(0,6):'';
    var tbr=f.tbr?Math.round(f.tbr)+'kbps':'';var size=f.filesize?fmtSize(f.filesize):'';
    return '<div class="fmt-row'+(S.fmtId===f.format_id?' selected':'')+'" data-fid="'+f.format_id+'"><div class="fmt-res">'+res+'p</div><div class="fmt-meta"><span>'+ext.toUpperCase()+(tbr?' &middot; '+tbr:'')+(size?' &middot; '+size:'')+'</span><span class="fmt-codec">'+codec+'</span></div><span class="fmt-badge">'+ext+'</span></div>';
  }).join('');
  document.querySelectorAll('.fmt-row').forEach(function(el){el.addEventListener('click',function(){S.fmtId=el.dataset.fid;renderFormats();});});
}
...
function renderDownloads(tasks){
  ...
  list.innerHTML=tasks.map(function(t){
    ...
    return '<div class="dl-item"><div class="dl-icon '+(isAudio?'audio':'video')+'">'+iconSvg+'</div><div class="dl-info"><h4>'+escHtml(t.title||t.task_id)+'</h4><p>'+dot+sub+'</p>'+prog+err+'</div><div class="dl-actions">'+actions+'</div></div>';
  }).join('');
  list.querySelectorAll('.dl-get').forEach(function(b){b.addEventListener('click',function(){
  ...
  list.querySelectorAll('.dl-del').forEach(function(b){b.addEventListener('click',function(){
    API.del(b.dataset.type,b.dataset.id).then(function(){toast('Deleted','info');loadDownloads();}).catch(function(){toast('Delete failed','error');});
  });});
}

function handleFetch(){
  var url=urlInput.value.trim();if(!url)return;
  S.url=url;S.info=null;S.fmts=[];S.fmtId=null;S.task=null;S.loading=true;updateUI();
  API.info(url).then(function(info){S.info=info;S.fmts=processFormats(info.formats||[]);S.fmtId=S.fmts.length?S.fmts[0].format_id:null;S.loading=false;updateUI();}).catch(function(e){S.loading=false;updateUI();toast(e.message||'Failed to fetch info','error');});
}

function handleDownload(){
  if(!S.url)return;btnDl.disabled=true;btnDl.classList.add('pulsing');
  var p=S.mode==='audio'?API.audio(S.url,S.preset):API.video(S.url,S.fmtId||null);
  p.then(function(res){S.task={id:res.task_id,type:S.mode};startPoll(res.task_id,S.mode);progSec.style.display='block';dlSec.style.display='none';toast('Download started','info');}).catch(function(e){btnDl.disabled=false;btnDl.classList.remove('pulsing');toast(e.message||'Failed to start download','error');});
}
...
function startPoll(id,type){
  ...
  polls[id]=setInterval(function(){
    API.status(type,id).then(function(s){updateProgress(s);
      if(s.status==='completed'){stopPoll(id);toast('Download complete!','success');btnDl.disabled=false;btnDl.classList.remove('pulsing');setTimeout(function(){progSec.style.display='none';S.task=null;dlSec.style.display='block';},2000);}
      else if(s.status==='failed'){stopPoll(id);toast('Download failed: '+(s.error||'Unknown'),'error');btnDl.disabled=false;btnDl.classList.remove('pulsing');document.getElementById('p-fill').classList.add('failed');}
    }).catch(function(){});},1000);
}
...
function loadDownloads(){
  API.tasks().then(function(tasks){renderDownloads(tasks);tasks.forEach(function(t){if(t.status==='downloading'||t.status==='processing'||t.status==='queued'){if(!polls[t.task_id])startPoll(t.task_id,t.type);}});}).catch(function(){toast('Failed to load downloads','error');});
}

function switchTab(tab){S.tab=tab;document.querySelectorAll('.page').forEach(function(p){p.classList.remove('active');});document.getElementById('page-'+tab).classList.add('active');document.querySelectorAll('.nav-btn').forEach(function(b){b.classList.toggle('active',b.dataset.tab===tab);});if(tab==='downloads')loadDownloads();}
function switchMode(mode){S.mode=mode;document.querySelectorAll('.mode-tab').forEach(function(t){t.classList.toggle('active',t.dataset.mode===mode);});document.getElementById('mode-ind').classList.toggle('right',mode==='video');audioOpt.style.display=mode==='audio'?'block':'none';videoOpt.style.display=mode==='video'?'block':'none';}

function toast(msg,type){
  type=type||'info';var el=document.createElement('div');el.className='toast '+type;
  var icons={success:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="width:16px;height:16px"><polyline points="20 6 9 17 4 12"/></svg>',error:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="width:16px;height:16px"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>',info:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="width:16px;height:16px"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>'};
  el.innerHTML=(icons[type]||icons.info)+'<span>'+escHtml(msg)+'</span>';
  document.getElementById('toasts').appendChild(el);setTimeout(function(){el.classList.add('out');setTimeout(function(){el.remove();},300);},3000);
}
...
document.getElementById('cookie-file').addEventListener('change',function(){
  ...
  API.cookiesUpload(fd).then(function(d){S.cookiesLoaded=true;updateCookieBadge();toast(d.message,'success');}).catch(function(e){toast(e.message||'Cookie upload failed','error');});
  this.value='';
});

urlInput.addEventListener('input',function(){var v=urlInput.value.trim();btnFetch.disabled=!v;btnClear.style.display=v?'flex':'none';});
btnPaste.addEventListener('click',function(){navigator.clipboard.readText().then(function(t){urlInput.value=t;urlInput.dispatchEvent(new Event('input'));}).catch(function(){toast('Cannot read clipboard','error');});});
btnClear.addEventListener('click',function(){urlInput.value='';urlInput.dispatchEvent(new Event('input'));S.info=null;S.fmts=[];S.fmtId=null;S.task=null;S.url='';updateUI();});
